...
// Vue-i18n 9's actual linked-message syntax is `@:` (or @.modifier:).
// The previous rule flagged ANY `@` followed by alphanumeric and pushed
// people toward `{'@'}` escape. Two problems with that:
//
//   1. Plain `@x` no longer crashes createI18n in 9.14.x — only logs a
//      "Invalid linked format" warning at boot and renders the literal
//      string correctly.
//   2. `{'@'}` only resolves when the value goes through t(); messages
//      pulled via tm() (raw) stay as `noc{'@'}example.com` literally.
//      Landing.vue uses tm('peering.policy') so the escape broke the
//      contact email rendering until 2026-05-28 (#email-format-fix).
//
// We now only flag the real linked-message syntax `@:key` (alphanumeric
// after `@:`) and let plain `@x` through. The boot warning is noise but
// not breakage. If you want it absolutely silent, split the address
// into local-part + '@' + domain in the template rather than relying
// on i18n escapes.
...
function checkValue(key, value) {
  ...
  if (AT_LINK_RE.test(value)) {
    out.push({
      key, value,
      msg: 'looks like a vue-i18n linked-message reference (@:key) — confirm the linked key exists, or split if you meant a literal address.',
    })
  }
