...
```
        ...
        └───────────────────────┬─────────────────────────┘  └─────────────┘
            ...
            ┌──────────┬─────────┼─────────┬───────────┐
            ▼          ▼         ▼         ▼           ▼
          PoP        PoP       PoP       PoP    …   (你有多少就多少)

   每个 PoP 运行 BIRD(BGP、任播),可选运行 ncn-agent(遥测)。
   角色 —— 控制 / 观测 / HA 备机 / RPKI / 边缘 —— 可自由组合:小部署塞一台,
   规模大就分散到多台。骨干是你自己的 PoP 间 IPv6 mesh。
```
...
| [`webmail/`](webmail/) | `ncn-mail` —— 服务 `mail.example.com` 的独立 Webmail(自包含)。 |
...
平台是一组**可自由组合的角色**,映射到你拥有的任意数量主机 —— 从单台到几十个
PoP 都行,**不假设固定节点数**。

| 角色 | 跑什么 | 数量 |
|---|---|---|
| **控制** | `ncn-api`、PostgreSQL 主库、nginx(控制台 / 公开 / Wiki 三 vhost) | 恰好一个;机群的唯一写入者 |
| **边缘 PoP** | BIRD(BGP / 任播),可选 `ncn-agent` | 你网络有多少就多少 —— 控制台只读它们 |
| **观测** | Prometheus + Grafana + Gatus | 可选;任意主机 |
| **HA 备机** | PostgreSQL 流复制备库 + 温备 `ncn-api` + `ncn-lb` 故障切换 | 可选;用于故障切换(见 [`DEPLOYMENT.md`](DEPLOYMENT.md)) |
| **RPKI** | Krill CA(发布 ROA)+ Routinator(校验) | 可选;若你跑 RPKI |
| **Webmail** | `ncn-mail` + Postfix/Dovecot | 可选;用于 `mail.example.com` |

- **小部署:** 控制(连同其它一切)塞一台,角色合并即可。
- **规模大:** 每个角色给独立主机,加一台备机做 HA。边缘 PoP 横向扩展:你运营
  多少就注册多少,跑在你自己的 IPv6 骨干上。
- 公开端点放在你自己的边缘 / CDN(TLS)后面。

(本仓库里 `ctrl-01` / `pop-01` 这类节点 ID 只是**示例占位**,你在节点注册表里随意命名。)
...
- **`ncn-lb`**(`lb/`)—— 带健康检查的源站池,自动主备切换(提升 PG 备库 →
  启动温备 `ncn-api` → 重指 Cloudflare DNS)。跑在备机上以扛住控制节点宕机。
...
| **HA 与灾备** | PostgreSQL 流复制(主库 → 备库,亚秒级 RPO);经 WAL 归档 + 每周基础备份 + 恢复演练的 PITR;`ncn-lb` 故障切换;每日状态快照(本地 + 异地)。 |
| **观测** | 手写的 Prometheus `/metrics`(不含密钥);某观测主机上的 Prometheus + Grafana(Grafana 经管理员限定的反代内嵌进控制台);Gatus uptime。 |
...
| **Webmail** | `ncn-mail` 承载 `mail.example.com`(到 dovecot/postfix 的 IMAP/SMTP 回环,经 rspamd 做 DKIM)。 |
...
- `ncn-api` 由 socket 激活(`ncn-api.socket` + `ncn-api.service`),所以切换是
  零停机的。
- 每日状态灾备快照(本地 + 异地主机)与审计日志异地 rsync 经 systemd 定时器
...
- 运行时配置与密钥位于控制节点的 `/etc/ncn-core-console/` 下(如 `session.key`、
...
- **机群访问**使用 控制节点上一把专用 SSH 密钥;远程命令执行经 agent 流程人工批准。
